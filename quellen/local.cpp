...
void AmericanOption::weights_erstellen(int ll) {
	for (int n = 0; n < N - 1; ++n)
		for (int l = 0; l < Mtraining(ll); ++l)
			weights[ll][n][l] = kernelD(X0, X[ll][l][n + 1],dt * (double) (n + 1));
...
double AmericanOption::C_estimate_Mesh(double* x, int Etime, int l) {
	...
	double int_dt = 0.1/(double)Mtraining(l);

	double v = EB.european_MaxCall_ND(x, D, (double) (Etime) * dt,	(double) (Etime + 1) * dt, Strike, r, delta, sigma[0], int_dt);
	...
	for (int k = 0; k < Mtraining(l); ++k) {
		...
		double CV = exp(-r * (double) (Etime + 1) * dt)	* std::max(Max(X[l][k][Etime + 1], D) - Strike, 0.);
